...
def cross_entropy_label_smoothing_loss(logits, labels, mask=None, label_smoothing=0.0):
...
def date_loss_l2(pred, target_min, target_max, mask):
    ...
    loss = 0.0
    loss += (pred - target_min) ** 2 * jnp.less(pred, target_min).astype(pred.dtype)
    loss += (pred - target_max) ** 2 * jnp.greater(pred, target_max).astype(pred.dtype)
...
def date_loss_l1(pred, target_min, target_max, mask):
    ...
    loss = 0.0
    loss += jnp.abs(pred - target_min) * jnp.less(pred, target_min).astype(pred.dtype)
