...
def date_loss_l1(pred, target_min, target_max):
    ...
    loss = 0.0
    loss += np.abs(pred - target_min) * np.less(pred, target_min).astype(pred.dtype)
    loss += np.abs(pred - target_max) * np.greater(pred, target_max).astype(pred.dtype)
    return loss
...
def saliency_loss_mask(forward, params, text_char_emb, padding, char_pos, char_idx):
