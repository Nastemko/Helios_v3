...
from vendor.predictingthepast.util import region_names
...
class RestorationResults(NamedTuple):
    ...
    def build_json(self):
        return {
            ...
            "prediction_saliency": [m.build_json() for m in self.prediction_saliency],
        }
...
def load_retrieval(path):
    ...
    emb_v_all_normed = (emb_v_all - retrieval["emb_v_mean"]) / retrieval["emb_v_std"]
