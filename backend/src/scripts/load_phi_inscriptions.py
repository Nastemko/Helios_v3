...
def load_phi_inscriptions(
    ...
    if not _phi_json_path.exists():
        logger.error(f"PHI JSON file not found: {_phi_json_path}")
        logger.info("Make sure iphi.json is in the iphi/train/data/ directory")
        return

    # Load JSON
    logger.info(f"Loading PHI data from {_phi_json_path}")
    with open(_phi_json_path, "r", encoding="utf-8") as f:
    ...
    try:
        ...
        for idx, insc in enumerate(inscriptions):
            try:
                ...
                if inserted % batch_size == 0:
                    db.commit()
                    logger.info(
                        f"Progress: {inserted} inserted, {skipped} skipped, {errors} errors ({idx + 1}/{len(inscriptions)})"
                    )

            except Exception as e:
                logger.warning(
                    f"Error processing inscription {insc.get('id', 'unknown')}: {e}"
                )
                errors += 1
                db.rollback()
                continue
        ...
        inscription_count = db.query(Text).filter(Text.urn.like("urn:phi:%")).count()
...
def clear_inscriptions():
    ...
    try:
        ...
        inscription_texts = db.query(Text).filter(Text.urn.like("urn:phi:%")).all()
