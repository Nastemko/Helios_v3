...
def populate_greek_texts(
    data_dir: Path, db: Session, limit: Optional[int] = None, force: bool = False
) -> dict:
    """
    ...
    """
    stats = {"inserted": 0, "skipped": 0, "errors": 0, "total_segments": 0}
...
def run_population(
    data_dir: Optional[str] = None, limit: Optional[int] = None, force: bool = False
) -> dict:
    """
    ...
    try:
        stats = populate_greek_texts(
            data_dir=data_path, db=db, limit=limit, force=force
        )
