...
PRED_PATH = Path(__file__).parent.parent.parent.parent / "predictingthepast_exp"
if PRED_PATH.exists():
...
class AeneasService:
    ...
    def _load_models(self):
        ...
        if all(p.exists() for p in [greek_checkpoint, greek_dataset, greek_retrieval]):
            try:
                ...
                self.models["greek"] = self._load_checkpoint(
                    greek_checkpoint, greek_dataset, greek_retrieval, "greek"
                )
        ...
        if all(p.exists() for p in [latin_checkpoint, latin_dataset, latin_retrieval]):
            try:
                ...
                self.models["latin"] = self._load_checkpoint(
                    latin_checkpoint, latin_dataset, latin_retrieval, "latin"
                )
        ...
        if not self.models:
            logger.warning(
                "No Aeneas models loaded. Download model files to enable AI features."
            )
